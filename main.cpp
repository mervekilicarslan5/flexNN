...
int main(int argc, char** argv)
{
	...
	if (cfg.mode == "train") {
		if (cfg.loss == "cross_entropy") {
			model.compile({ cfg.batch, ch, h, w }, new SGD(cfg.lr, cfg.decay), new CrossEntropyLoss<DATATYPE>);
		}
		else {
			model.compile({ cfg.batch, ch, h, w }, new SGD(cfg.lr, cfg.decay), new MSELoss<DATATYPE>);
		}
